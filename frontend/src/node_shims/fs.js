...
function importVfsBackup() {
    const i = DOM.createElement("input", {type: "file"});
...
function upgradeVfsData() {
    ...
    for (const [key, value] of Object.entries(cache.data)) {
        ...
        if (Object.getOwnPropertyNames(vfsObject).some((e) => propertiesToRemove.includes(e))) {
            Logger.log("VFS", `❌ Removing obsolete VFS metadata for: ${vfsObject.fullName}.`);

            propertiesToRemove.forEach(prop => {
                if (vfsObject.hasOwnProperty(prop)) {
                    Logger.log("VFS", `  Removing property "${prop}".`);
        ...
        if (vfsObject.nodeType === "file" && vfsObject.contents.constructor.name === "String") {
            Logger.log("VFS", `♻ Converting "contents" from String to Uint8Array for: ${vfsObject.fullName}.`);
        ...
        if (hasChanged) {
            Logger.log("VFS", "✅ Committing entry to data store.");
...
function inotify(path, event, source) {
    ...
    const newSource = Path.basename(path);
